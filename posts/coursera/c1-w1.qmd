...
keywords: [the k-armed bandit problem, bandit algorithms, exploration, exploitation, epsilon greedy algorithm, sample average method]
jupyter: python3
...
![bandit](img/multi_armed_bandit.webm){.column-margin group="slides"}
...
![clinical trial](img/rl-clinical-trial.png){.column-margin group="slides"}
...
$$
\begin{align}
q_{\star}(a) & \doteq \mathbb{E}[R_t  \vert  A_t=a] \space \forall a \in \{a_1 \ldots a_k\} \\  
             & = \sum_r p(r \mid a)r \qquad \text{(action value)}
\end{align}
$$ {#eq-action-value}
...
![decisions](img/rl-descion-problems.png){.column-margin group="slides"}
...
![why discuss bandits](img/rl-why-bandits.png){.column-margin group="slides"}
...
![estimating action values](img/rl-clinical-trial-q(a).png){.column-margin group="slides"}
...
![example](img/rl-sample-avarage-method.png){.column-margin group="slides"}
...
{{< include _c1-w1_alg-simple-epsilon-greedy-bandit.qmd >}}
...
{{< include _c1-w1_alg-greedy-bandit.qmd >}}
...
{{< include _c1-w1_alg-Ɛ-greedy-bandit.qmd >}}
...
{{< include _c1-w1_alg-optimistic-greedy-bern-bandit.qmd >}}
...
![The effect of optimistic initial action-value estimates](img/rl-optimistic-initial-conditions.png){.column-margin group="slides"}
...
$$
A_t = \arg\max_a \Bigg[
  \underbrace{Q_t(a)\vphantom{\sqrt{\frac{\ln t}{N_t(a)}} }}_{exploitation} + 
...
![UCB intuition](img/rl_wk1_ucb.png){.column-margin  group="slides"}
...
{{< include _c1-w1_alg-ucb-bandit.qmd >}}
...
{{< include _c1-w1_alg-bernoulli-thompson-sampling.qmd  >}}
...
class EpsilonGreedyAgent(Agent):
  ...
  def __init__(self, model, num_arms, epsilon=0.1):
  ...
  def step(self):
      ...
      reward = self.model.get_reward(chosen_arm)
      assert reward is not None, "Reward is not provided by the model"
      self.action_counts[chosen_arm] += 1
      step_size = 1 / self.action_counts[chosen_arm]
      self.q_values[chosen_arm] += step_size * (reward - self.q_values[chosen_arm])
...
class TestbedModel(Model):
  ...
  def __init__(self, num_arms, mean_reward, std_dev,num_agents=1):
      ...
      for _ in range(self.num_agents):
        self.create_agent(EpsilonGreedyAgent, 0.1)

  def env_init(self,env_info={}):
      self.arm_means = np.random.normal(self.mean_reward, self.std_dev, size=self.num_arms)

  def create_agent(self, agent_class, epsilon):
      """
      Create an RL agent instance with the specified class and parameters.
      """
      return agent_class(self, self.num_arms, epsilon)

  def step(self):
      self.agents.shuffle_do("step")

  def get_reward(self, arm_id):
      return np.random.normal(self.arm_means[arm_id], 1.0)
