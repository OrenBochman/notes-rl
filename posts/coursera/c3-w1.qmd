...
author: Oren Bochman
draft: false
categories: [Coursera,Prediction and Control with Function Approximation,Reinforcement Learning]
...
I did not find the derivation of the SGD alg particularly enlightening and I have seen it several times. 
However the online setting is the best motivation for the use of SGD and makes perfect sense in the context of reinforcement learning. 
Minibatches are then a natural extension of this idea.
...
The idea is that using a table has some limitations. 
The first is that the tables can be very large. 
For continuous states they can become infinite.

The second is that the tables can be very sparse and in a table we don't generalize between states.

We see that we don't really want functions that directly approximate the value function. 
We want functions that have some structure that we can learn.

This is called a **parameterized function**. 
The idea is to use a weighted sum of the features of the state. 
This allows us to learn the weights and use them to approximate the value function. 
This is called **linear function approximation**.
The way to get around this which is two fold. 
We first represent the salient properties of a states into features. 

Then we use weights to combine these features to approximate the value function. 

More generally we can use non linear parameterized functions to approximate value functions.

Adam shows that is the features are not picked wisely we may not be able to discriminate between states - out function for one state will be the same as for another dissimilar state. 
Learning about one will make us forget what we learned about the other. 
This is called **bias**. 
On the other hand if we have too many features we may not be able to generalize between states. 
This is called **variance**. 

The goal is to balance between **bias** and **variance**.
...
-   In the previous courses we represented value functions as tables or arrays:
    ...
    -   For $Q(s,a)$ we had an array of size $|S| \times |A|$. This becomes impractical as $\mid S \mid \rightarrow \infty$. We can use **parameterized functions** to approximate value functions. This is called **function approximation**.
...
we get a sequence of $(S_1,v_{\pi}(S_1) ),(S_2,v_{\pi}(S_2) ), (S_3,v_{\pi}(S_3) ), \ldots$ and we want to approximate the value function . We can track how well we are approximating $v_{\pi}(s)$ by using $\hat{v}(s,\mathbf{w})$. The difference can be positive or negative so if we average it the sum will tend to cancel out. If we square the error we get a positive number we have a much better estimate of the error. And if normalize it by taking the mean we can get use it to compare runs of different lengths. This is called the mean squared  error.
...
$$
\begin{aligned}
w_{t+1} & \doteq w_t + \alpha [v_\pi(S_1) - \hat{v}(s_1, \mathbf{w_1})]\nabla \hat{v}(s, \mathbf{w}) \\
        & \doteq w_t + \alpha [G_1 - \hat{v}(s_1, \mathbf{w_1})]\nabla \hat{v}(s, \mathbf{w})
\end{aligned}
...
{{< include /posts/coursera/_c3-w1_alg1.qmd >}}
...
{{< include /posts/coursera/_c3-w1_alg2.qmd >}}
...
-   This might be happen through planning but ideally, agents should be able to learn skills which are sequences of actions that are executed over an extended period of time.
...
-   This type of formulation seems very similar to that used by [Judea Pearl](https://www.youtube.com/watch?v=JxzjdTc15A0) in his structural graphical model of Causality. If we can express options as a graph of states we can use his algorithms to infer the best options to take in a given state.
-   options are like do operations (interventions)
-   choosing between options is like counterfactual reasoning.
:::
