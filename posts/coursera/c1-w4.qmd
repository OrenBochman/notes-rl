...
The *programming* in dynamic programming really means solving an optimization problem. 
We have learned about using the Bellman equations as update rules. Now we look at some basic applications of this idea to solve MDP.

The intuition is pretty simple we have two tasks 
  - one is to decide how good a policy $\pi$ is -- think [discounted summation of the rewards from the best actions over the $s_ta_tr_t$ tree]{.mark}. 
This policy evaluation step is named **Prediction**, as we don't really know what the actual rewards of stochastic actions will be, only their Expectation. But what we really want is to find near optimal policy which is called '**Control**'. We have a strong theoretical result on how to go about this by iteratively improving a policy by picking its the actions with highest value at each steps.
...
$$ {#eq-iterative-policy-evaluation-update}
...
{{< include _c1-w4_alg-iterative-policy-evaluation.qmd >}}
...
It is an iterative algorithm that computes the optimal value function and the  optimal policy for a given MDP but it does not directly reference a particular policy. 

In value iteration, the algorithm starts with an initial estimate of the value function  and iteratively runs a single step of greedy policy evaluation per step, using the greedy value to update the state-value function.

Updates the value function until it converges to the optimal value function.

{{< include _c1-w4_alg-policy-iteration.qmd >}}


### The Dance of Policy and Value {#sec-l2g4}

![Dance of Policy and Value](img/rl-dance.png){.column-margin group="slides"}
...
Generalized policy iteration is a framework for solving reinforcement learning problems that combines policy evaluation and policy improvement in a single loop. 
The idea is to alternate between evaluating the policy and improving the policy until the policy converges to the optimal policy.

### Value Iteration {#sec-l3g2}

Value iteration is an important example of generalized policy iteration. It is an iterative algorithm that computes the optimal value function and the optimal policy for a given MDP. 
The algorithm starts with an initial estimate of the value function and iteratively updates the value function until it converges to the optimal value function.
...
iteration, while asynchronous dynamic programming methods update only a subset of states in each iteration. 
Synchronous dynamic programming methods are typically slower than asynchronous dynamic programming methods, but they are guaranteed to converge to the optimal policy.
...
Brute force search is computationally expensive and is not practical for large MDPs.

### Monte Carlo {#sec-l3g5}

Monte Carlo is an alternative method for learning a value function. It involves estimating the value function by sampling returns from the environment. 
Monte Carlo is computationally expensive and is not practical for large MDPs.

::: {.callout-tip}
## Variational Inference for Large MDPs

**Update** Since learning this lecture I studied Variational Inference which is a modern method for approximating complex probability distributions and allows us to scale up MCMC using optimization with a mean field assumption AKA MFVI. Revisiting this note I wonder if this and related methods have been used to overcome the above mentioned limitation of classic MCMC for finding optimal policies in large MDPs.

1. VAI (Variational Bayeian Inference) that supports scaling up MCMC using optimization with a mean field assumption.
  -  c.f. Variational Policy Iteration (VPI) that can be used to find the optimal policy in large MDPs. 
  - [@chow2020variational] Variational Model-based Policy Optimization (VMBPO)
  - [@meyer2024warm] Warm-Start Variational Quantum Policy Iteration (WS-VarQPI)
  - [@levine2013variational] Variational Policy Search via Trajectory Optimization
  - [@salkuyeh2016interpolated] Variational Value Iteration (VVI)
1. Further speedups by preprocessing using coresets to reduce  N for large tabular settings
1. further flexibility by using a coreset tree
1. using the online variants ie.e SVI that might be more inline with RL.
1. using a Posterior Corrections e.g. Linear Response Variational Bayes (LRVB) to overcome the issues for VAE underestimating variance or missing mean.
...
Dynamic programming and bootstrapping are more efficient than brute force search and Monte Carlo for finding the optimal policy. 
Dynamic programming and bootstrapping exploit the structure of the MDP to update the value function iteratively, while brute force search and Monte Carlo do not.
